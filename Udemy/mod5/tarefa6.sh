...
# ------------------------------- TESTES ----------------------------------------- #     
test -z "$1"  && echo -e $VERMELHO'Por favor passe uma flag!'


while test -n "$1"; do
   case "${1}" in
        -h)
            echo -e $VERDE"${HELP}" && exit 0                                                                                     ;; 
        -v | --version)
            echo -e $VERDE"${VERSION}" && exit 0                                                                                  ;; 
        -r)
            REPOS=1                                                                                                      ;;
        -p)
            PACKAGES=1                                                                                                   ;;
        -c)
            CLEAN=1                                                                                                      ;;
        --all)
            sudo apt-get update -y && sudo apt-get upgrade -y && sudo apt-get clean && echo -e $VERDE"Tudo Pronto" && exit 0      ;;
        *)
           echo -e $VERMELHO'Por favor passe um parametro valido ou consulte o menu de ajuda com a flag -h'    ;;
   esac
   shift
done
